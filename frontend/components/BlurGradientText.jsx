...
export default function BlurGradientText({
  ...
  useAnimationFrame(time => {
    ...
    if (yoyo) {
      const sine = Math.sin((elapsedRef.current / animationDuration) * Math.PI - Math.PI / 2);
      progress.set(((sine + 1) / 2) * 100);
    } else {
    ...
    <p ref={ref} className={`inline-flex flex-wrap ${className}`}>
      {elements.map((segment, index) => {
        ...
        return (
          <motion.span
            key={index}
            className="inline-block will-change-[transform,filter,opacity] pb-[0.2em] -mb-[0.2em] pt-[0.2em] -mt-[0.2em]"
            style={{
              backgroundImage,

...
const GROQ_API_KEY = process.env.NEXT_PUBLIC_BLOG_GEN_GROQ_API_KEY;
...
export const callGroq = async (systemPrompt: string, userPrompt: string, model = 'openai/gpt-oss-20b', maxTokens = 1200, jsonMode = false) => {
    ...
    while (retries <= maxRetries) {
        try {
            ...
            const response = await groqClient.post('', payload);
